...
def create_app(session_store: TemporarySessionStore) -> gr.Blocks:
    def run(
        ...
        try:
            ...
            if not edge_file:
                raise ValueError("Please upload a spectrum similarity edge table.")
            edge_frame = read_similarity_edges(edge_file)
            if edge_frame.empty:
        ...
        payload = {
            ...
            "molecular_network_job": {
                "documents": documents,
                "edge_tsv": edge_frame.to_csv(sep="\t", index=False),
    ...
    with gr.Blocks(title="MSP Molecular Network + KG") as app:
        with gr.Group(elem_classes="massbank-page"):
            ...
            edge_file = gr.File(
                label="Spectrum similarity edges (TSV/CSV)",
            ...
            run_button.click(
                run,
                [
                    msp_files, file_classes, edge_file, resume_enabled, top_n,
